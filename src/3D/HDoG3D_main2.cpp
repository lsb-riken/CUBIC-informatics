...
void initializeGPU() {
  ...
  d_dog = d_norm;//reinterpret_cast<float*>(d_labels_tmp);
...
void processSubstack(const int i_stack, const int z0) {
  ...
  if (!p.is_ave_mode) {
    MaxNormalized(d_labels, d_dog, d_labels_tmp, d_norm_tmp,
                  d_labels_region, d_maxnorm_region,
                  d_cub_tmp, p.cub_tmp_bytes,
                  d_num_regions, p.width, p.height, p.depth);
  } else {
    SumNormalized(d_labels, d_dog, d_labels_tmp, d_norm_tmp,
                  d_labels_region, d_maxnorm_region,
                  d_cub_tmp, p.cub_tmp_bytes,
                  d_num_regions, p.width, p.height, p.depth);
  }
  ...
  saveFeatureVector(h_maxnorm_region,
                    ...
                    h_num_regions, depth, z0, p, "ab", -10.0);
